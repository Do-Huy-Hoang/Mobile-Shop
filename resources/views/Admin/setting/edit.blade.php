...
@section('content')
    ...
    @include('Admin.pages.content-header', ['name' => 'Settings' , 'key' => 'edit'])
    <!-- /.content-header -->
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="panel-body">
                            <form class="add" method="post" action="{{route('setting.update', ['id'=>$setting->id])}}">
                            @csrf
                            <!-- Config key-->
                                ...
                                @if(request()->type === 'Text')
                                    <div class="form-group">
                                        ...
                                        <input require="true" type="text" class="form-control @error('config_value') is-invalid @enderror" id="config_key"
                                               name="config_value" placeholder="Enter Config value" value="{{$setting->config_value}}">
                                    </div>
                                    @error('config_value')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                    @enderror
                                @elseif(request()->type === 'Textarea')
                                    <div class="form-group">
                                        ...
                                        <textarea require="true" type="text" class="form-control @error('config_value') is-invalid @enderror"
                                                  id="config_key" name="config_value" rows="4">{{$setting->config_value}}</textarea>
                                    </div>
                                    @error('config_value')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                    @enderror
