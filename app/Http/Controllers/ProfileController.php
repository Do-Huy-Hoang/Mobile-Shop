...
use App\Http\Requests\CustomerUpdateResquest;
use App\Http\Requests\PhoneUpdateResquest;
use App\Http\Requests\UpdatePassResquest;
...
class ProfileController extends Controller
{
    ...
    public function profileupdate($id, CustomerUpdateResquest $request){
        try {
            DB::beginTransaction();
            $customer = $this->customer::find($id);
                $this->user::find($customer->user_id)->update([
                        'name'=>$request->name_login,
                ]);
    ...
    public function PhoneUpdate($id,PhoneUpdateResquest $request){
        try {
            DB::beginTransaction();
            $this->customer::find($id)->update([
                'phone' => $request->phone,
    ...
    public function PasswordUpdate($id,UpdatePassResquest $request){
        $settings = $this->settings::all();
        $category = $this->category::all();
        $user = $this->user::find(auth()->id());
        $error = false;
